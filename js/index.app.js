...
var Application = function(json_data_source) {
	...
	.then(function(model) { 
		...
		// Capture overall numbers
		var overview = {};

    	var moduleWithIssue = 0;
    	var totalmodules = 0;
    	var totalcomponents = 0;
    	var componentWithIssue = 0;
		
		// Each group
		for (var j = 0; j < data.length; j++) {
        	var data_item = data[j];
			var name = data_item.name;
			console.log(name);
			var files = data_item.files;
			overview[name] = {'compiler':0, 'misra_7':0, 'misra_8' : 0, 'coverity':0, 'files': 0, 'size': 0};
			...
			for (var i=0; i < files.length; i++) {
				...
				if (file.compiler_issues.length > 0 ||
					...
					html_group += 
							...
							'<td></td><td><div class="glyphicon glyphicon-chevron-right" style="margin-right:10px"> </div>' + file.path + '</td>' + 
			...
			//if (cissues) 
			{
				var html_total =
					...
					"<td class=\"number\">" + emptyIf0(overview[name].misra_7)  + "</td>" + 
					"<td class=\"number\">" + emptyIf0(overview[name].misra_8)  + "</td>" + 
			...
			$("#comps").append('<tr class="issuegroupexpander" style="background-color:#eeeeee" data-group-index="'+ j + '"' + 
	...
	$('.issuegroupexpander').on('click', '', function(event){
        ...
        Object.keys(overview).forEach(function(o) {
			data_comp.push({'label': o, 'value' : overview[o].compiler, 'color' : overview[o].color, 'labelFontSize': '10', 'labelColor':'white', 'labedl':'', 'highlights' :overview[o].color});
			comp_sum_cnt += overview[o].compiler;
			data_cov.push({'label': o, 'value' : overview[o].coverity, 'color' : overview[o].color, 'highlights' : overview[o].color});
			cov_sum_cnt += overview[o].coverity;
			data_misra.push({'label': o, 'value' : overview[o].misra_8, 'color' : overview[o].color, 'highlights' : overview[o].color});
			misra_sum_cnt += overview[o].misra_8;
			data_size.push({'label': o, 'value' : overview[o].size, 'color' : overview[o].color, 'highlights' : overview[o].color});
			size_cnt += overview[o].size;
			data_files.push({'label': o, 'value' : overview[o].files, 'color' : overview[o].color, 'highlights' : overview[o].color});
			files_cnt += overview[o].files;
			if (stats[o] !== undefined) {
				linesofcode.push({'label': o, 'value' : stats[o], 'color' : overview[o].color, 'highlights' : overview[o].color});
